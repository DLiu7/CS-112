...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        for (int i = 0; i < numCategories; i++) {
            ...
            int numDish = StdIn.readInt();

            for (int g = 0; g < numDish; g++) {
                String emptySpace = StdIn.readLine();
                String dishName = StdIn.readLine();
                int numIngred = StdIn.readInt();
                int[] stockIDs = new int[numIngred];

                for (int j = 0; j < numIngred; j++) {
                    stockIDs[j] = StdIn.readInt();
                }

                Dish dish = new Dish(categoryName, dishName, stockIDs);
                MenuNode dishNode = new MenuNode(dish, null);
    ...
    StockNode ptr = stockVar[newNode.getIngredient().getID() % stockVarSize];
        newNode.setNextStockNode(ptr);
        stockVar[newNode.getIngredient().getID() % stockVarSize] = newNode;
    ...
    StockNode ptr = null;
        ...
        if (ptr != null) {
            int newStock = ptr.getIngredient().getStockLevel() + stockAmountToAdd;
            ptr.getIngredient().setStockLevel(newStock);
        }
    ...
    for (int i = 0; i < menuVar.length; i++) {
        ...
        while (ptr != null) {
            double totalPrice = 0;
            int[] ingredientIDs = ptr.getDish().getStockID();

            for (int j = 0; j < ingredientIDs.length; j++) {
                StockNode stockNode = findStockNode(ingredientIDs[j]);
                totalPrice += stockNode.getIngredient().getCost();
            }

            double price = totalPrice * 1.2;
            ptr.getDish().setPriceOfDish(price);

            double profit = price - totalPrice;
    ...
    stockVarSize = StdIn.readInt();
        ...
        while (!StdIn.isEmpty()) {
            ...
            char hold = StdIn.readChar();
         
            String ingredientName = StdIn.readLine();
            double ingredientCost = StdIn.readDouble();
            int ingredientStock = StdIn.readInt();
         
            Ingredient ingredient = new Ingredient(ingredientID, ingredientName, ingredientStock, ingredientCost);
            StockNode newStockNode = new StockNode(ingredient, null);
         
            int index = ingredientID % stockVarSize;
            newStockNode.setNextStockNode(stockVar[index]);
            stockVar[index] = newStockNode;
        }
    ...
    MenuNode menuNode = findDish(dishName);
        ...
        for (int i = 0; i < ingredientIDs.length; i++) {
            StockNode stockNode = findStockNode(ingredientIDs[i]);
    
            if (stockNode == null) {
                return false;
            }
    
            int requiredQuantity = numberOfDishes;
    
            int availableQuantity = stockNode.getIngredient().getStockLevel();
	...
	// WRITE YOUR CODE HERE
    boolean ifProfitExceed = profit() > 50;
    boolean donationVerify = false;

    if(ifProfitExceed == true){

        StockNode ingredientNode = findStockNode(ingredientName);
        double ingredientStock = ingredientNode.getIngredient().getStockLevel();

                if (ingredientNode != null &&  ingredientStock >= quantity) {
                    updateStock(ingredientName, -1, -quantity);
                    donationVerify = true;
                }
    }
            
    addTransactionNode(new TransactionData("donation", ingredientName, quantity, 0, donationVerify));
    ...
    double totalCost = stockNode.getIngredient().getCost() * quantity;

    if (profit() >= totalCost) { 
        double totalProfit =- totalCost;

        TransactionData SuccessfulRestock = new TransactionData("restock", ingredientName, quantity, totalProfit, true);
        addTransactionNode(SuccessfulRestock);

        updateStock(ingredientName, quantity, quantity);
    } else {
        TransactionData unsuccessfulRestock = new TransactionData("restock", ingredientName, quantity, 0, false);
        addTransactionNode(unsuccessfulRestock);
    }
